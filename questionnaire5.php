...
<body class="bg-white">
  <div class="container mx-auto py-8 px-4">
    <h1 class="text-3xl font-bold text-center mb-8">Questionnaire 5</h1>
    <form action="traitement_reponses5.php" method="post" class="max-w-lg mx-auto">
      <?php
      // Connexion à la base de données
      require_once "config.php";

      // Récupérer les questions depuis la base de données
      $sql = "SELECT id, question FROM questions5";
      ...
      if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          $choices_sql = "SELECT id, choix FROM choix_reponses5 WHERE id_question=" . $row["id"];
          ...
          if ($choices_result->num_rows > 0) {
            ...
            while ($choice_row = $choices_result->fetch_assoc()) {
              ...
              echo "<input type='radio' name='reponse5_" . $row["id"] . "' value='" . $choice_row["id"] . "' class='mr-2' id='reponse5_" . $row["id"] . "_" . $choice_row["id"] . "'>";
              echo "<label for='reponse5_" . $row["id"] . "_" . $choice_row["id"] . "' class='text-lg'>" . $choice_row["choix"] . "</label>";
              echo "</div>";
            }
          }
          echo "</div>";
