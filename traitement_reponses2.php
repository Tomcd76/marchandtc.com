...
<head>
    ...
    <title>Résultats du questionnaire 2</title>
    <link rel="stylesheet" href="questionnaire.css">
</head>
<body class="bg-white">
  <div class="container mx-auto py-8 px-4">
    <h1 class="text-3xl font-bold text-center mb-8">Résultats du questionnaire 2</h1>
    <link rel="stylesheet" href="questionnaire.css">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      foreach ($_POST as $key => $value) {
        // Vérifier si le champ est une réponse à une question du questionnaire 2
        if (strpos($key, "reponse2_") === 0) {
          // Extraire l'ID de la question
          $question_id = substr($key, strlen("reponse2_"));
          ...
          $sql_correct_answer = "SELECT reponse_correcte FROM questions2 WHERE id = $question_id";
          ...
          if ($result_correct_answer && $result_correct_answer->num_rows > 0) {
            ...
            // Insérer la réponse de l'utilisateur dans la table reponses_utilisateurs2
            $sql_insert_reponse = "INSERT INTO reponses_utilisateurs2 (id_question, reponse_utilisateur, correcte) VALUES ('$question_id', '$reponse_utilisateur', '$correcte')";
      ...
      $nombre_questions = 8; // Remplacez 8 par le nombre total de questions dans le questionnaire 2
      $nombre_correct = 0;
...
$sql_correction_questions = "SELECT q.question, c.choix FROM questions1 q INNER JOIN choix_reponses1 c ON q.reponse_correcte = c.id";
$result_correction_questions = $conn->query($sql_correction_questions);
// Récupérer la correction de chaque question à choix multiple
$sql_correction_questions = "SELECT q.question, c.choix FROM questions2 q INNER JOIN choix_reponses2 c ON q.reponse_correcte = c.id";
$result_correction_questions = $conn->query($sql_correction_questions);
