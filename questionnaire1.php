...
<body class="bg-white">
  <div class="container mx-auto py-8 px-4">
    <h1 class="text-3xl font-bold text-center mb-8">Questionnaire 1</h1>
    <form action="traitement_reponses1.php" method="post" class="max-w-lg mx-auto">
      <?php
      // Connexion à la base de données
      require_once "config.php";

      // Récupérer les questions depuis la base de données
      $sql = "SELECT ID, question FROM questions1";
      ...
      if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          $choices_sql = "SELECT ID, choix FROM choix_reponses1 WHERE id_question=" . $row["ID"];
          ...
          if ($choices_result->num_rows > 0) {
            ...
            while ($choice_row = $choices_result->fetch_assoc()) {
              ...
              echo "<input type='radio' name='reponse1_" . $row["ID"] . "' value='" . $choice_row["ID"] . "' class='mr-2' id='reponse1_" . $row["ID"] . "_" . $choice_row["ID"] . "'>";
              echo "<label for='reponse1_" . $row["ID"] . "_" . $choice_row["ID"] . "' class='text-lg'>" . $choice_row["choix"] . "</label>";
              echo "</div>";
            }
          }
          echo "</div>";
